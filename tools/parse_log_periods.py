...
class pretty_name:
    ...
    def missing(self):
        ...
        if not self.parts[1]: res.append('period')
        #
...
def main(ofname, ifnames):
    ...
    with open(ofname, 'w') as fo:
        print(common_name.csv_header() + f' , blocks, time, 1-0 % , mDB % , B/sec', file=fo)
        #
        for name, res in zip(names, all_res):
            ...
            for j, row in enumerate(data['rows']):
                vs = row.get('vs', {})
                if 1 in vs and 4 in vs:
                    _, _, cnt1, tot1 = vs[1]
                    _, _, cnt4, tot4 = vs[4]
                    assert cnt1 == cnt4, (cnt1, cnt4)
                    if 't' in row:
                        t = row['t'] - t0
            ...
            t  = y1 + y4 + 1
            #
            print(name.csv() + f' , {cnt1:6}, {t[-1]/1e6:4.0f}, {100*y1[-1]/t[-1]:5.2f} , {100*tot71/t[-1]:5.2f} , {1e6*x[-1]/t[-1]:5.1f}', file=fo)
            #
            # dx = x[4:] - x[:-4]
            # dt = t[4:] - t[:-4]
            # #
    ...
    #
    ax.grid()
    ax.set(xlabel='Blocks', ylabel='blocks / second', title=common_name.full_title())
