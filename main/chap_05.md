...
Η ανάλυση γίνεται στατικά ^1, ξεκινώντας με ? disassembly ? του EVM κώδικα που βρίσκεται σε δυαδική αναπαράσταση (*bytecode*).
ΣΧΗΜΑ

Μετά χωρίζεται σε basic block. Αρκετά χρήσιμη για αυτό το στάδιο είναι η επιβολή της εντολής *JUMPDEST* του EVM
σε όλες τις διευθύνσεις στις οποίες επιτρέπεται να φτάσει η ροή της εκτέλεσης από εντολή άλματος.
ΣΧΗΜΑ

Στη συνέχεια, οι εντολές που περιέχουν τα basic block μετατρέπονται σε μορφή Static Single Assignment (*SSA*) [ REF ].
ΣΧΗΜΑ
...
εξαλείφοντας εντολές όπως *PUSH*, *POP* και *SWAP*, ενώ δημιουργούνται νέες πσευδο-εντολές Φ (phi) [ REF ].
...
Χρησιμοποιεί τον αλγόριθμο *worklist* [ REF ] για να λύσει, μεταξύ άλλων, τα παρακάτω dataflow προβλήματα [ REF is dataflow analysis ]:
- Αντικατάσταση σταθερών (constant folding) [ REF ]
- Αλγεβρική απλοποίηση [ REF ]
- Σύνολα πιθανών τιμών [ REF is dataflow analysis ]
...
Ο διαχωρισμός των δύο τμημάτων, εκτός από την αντιπροσώπευση των αλλαγών στη βάση κώδικα (*code base*), δείχνει και την διαφορά της βεβαιότητας της προανάκτησης:
...
[ FAS ] https://en.wikipedia.org/wiki/Feedback_arc_set

[ GOCHAN ] https://go.dev/ref/spec#Channel_types

[ SSA ] R. Cytron, J. Ferrante, B. K. Rosen, M. N. Wegman, and F. K. Zadeck. Efficiently computing static single assignment form and the control dependence graph. ACM Transactions on Programming Languages and Systems (TOPLAS)

[ LMDB ] http://www.openldap.org/pub/hyc/mdb-paper.pdf

[ MDBX ] https://github.com/erthink/libmdbx

[ METADATA ] https://docs.soliditylang.org/en/v0.8.12/metadata.html
