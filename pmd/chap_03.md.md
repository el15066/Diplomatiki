...
χρειάζεται να διατηρεί μια συνεπή (consistent) και αντιγραφόμενη (replicated) δομή με την κατάσταση (World State) των λογαριασμών, συμβολαίων και του αποθηκευτικού χώρου των συμβολαίων (contract storage ή απλά storage).
...
```go
Geth@12f0ff4/core/state/statedb.go:59-65,77-80
```


Δημιουγούνται διαβάζοντας το αντίστοιχο *Account* από τη *stateDB*:
```go
Geth@12f0ff4/core/state/statedb.go:489-493,503,513,517-518,535-537,541
```
...
```go
Geth@12f0ff4/core/state/snapshot/difflayer.go:270-273,291-295,311-312,318,344-346,349
```


Αν αποτύχει:
```go
Geth@12f0ff4/core/rawdb/accessors_snapshot.go:75-79
```
...
```go
Geth@12f0ff4/trie/trie.go:116-117,124,141-142,148-149
```
...
```go
Geth@12f0ff4/trie/trie.go:499-501
```

```go
Geth@12f0ff4/trie/database.go:372,393-394
```

```go
Geth@12f0ff4/ethdb/leveldb/leveldb.go:22,33,61,63,187-194
```


Τα *stateObject* ανακτούν δεδομένα *Storage* με τον ίδιο τρόπο:
```go
Geth@12f0ff4/core/state/state_object.go:180,191,195,224,237,238,250,
```


Οι ανακτήσεις δεδομένων *Storage* προκαλούνται από εντολές SLOAD/SSTORE κατά την εκτέλεση κάποιου *Smart Contract*:
```go
Geth@12f0ff4/core/vm/instructions.go:516-531
```
...
```go
Geth@12f0ff4/core/state_processor.go:59,76,88-93,
```

```go
Geth@12f0ff4/consensus/ethash/consensus.go:587-593,
```

```go
Geth@12f0ff4/core/state/statedb.go:823-879,
```

```go
Geth@12f0ff4/core/state/statedb.go:450-451,459,463,
```


Τέλος, η εγγραφή όλων των τροποποιήσεων στη βάση στο δίσκο γίνεται αργότερα, με κλήση στο *Trie*:
```go
Geth@12f0ff4/trie/trie.go:517,527,547
```
...
```go
Erigon@inline_uv/core/state/intra_block_state.go:50-85
```


Η δημιουργεία του *stateObject* γίνεται πάλι με ανάκτηση του αντίστοιχου *Account*:
```go
Erigon@inline_uv/core/state/intra_block_state.go:553-554,557,559,560,569,
```


Η διαφορά γίνεται εμφανής στην ανάγνωση του *Account* μιας και εδώ δεν υπάρχει δομή *Trie*:
```go
Erigon@inline_uv/core/state/plain_state_reader.go:70,73
```
...
```go
Erigon@b1fef26/ethdb/olddb/mutation.go:126-135,
```

```go
Erigon@b1fef26/ethdb/olddb/mutation.go:75-80
```

```go
Erigon@b1fef26/ethdb/olddb/mutation.go:3,13,20-22
```

```go
Erigon-lib@143cd51/kv/mdbx/kv_mdbx.go:872-879
```


Τα *stateObject* ανακτούν δεδομένα *Storage* με τον ίδιο τρόπο:
```go
Erigon@inline_uv/core/state/state_object.go:159-160,162,174,
```

```go
Erigon@inline_uv/core/state/state_object.go:184,186,188,211
```

```go
Erigon@inline_uv/core/state/plain_state_reader.go:103,108
```


Όπως είδαμε και προηγουμένως, οι ανακτήσεις δεδομένων *Storage* προκαλούνται από εντολές SLOAD/SSTORE:
```go
Erigon@inline_uv/core/vm/instructions.go:544-562
```


Επίσης, οι ανακτήσεις για *Accounts*, που γίνονται κατά την ολοκλήρωση της επεξεργασίας ενός *Block*:
```go
Erigon@inline_uv/core/blockchain.go:90,123,159,178,
```

```go
Erigon@inline_uv/core/blockchain.go:248,275,287
```

```go
Erigon@inline_uv/core/state/intra_block_state.go:287-289
```

```go
Erigon@inline_uv/core/state/plain_state_writer.go:125,127
```
...
```go
Erigon@inline_uv/eth/stagedsync/stage_execute.go:200,228-232
```


Αν ενεργοποιηθεί, οι εγγραφές γίνονται με τον *change_set_writer*:
```go
Erigon@inline_uv/core/state/change_set_writer.go:119,120,124,125,133,140,141
```


Η συνάρτηση *Encode()* κάνει απλή μετάφραση και ταξινόμηση:
```go
Erigon@inline_uv/common/changeset/account_changeset.go:23-35
```
